...
#         #     f = self.kconv4_3(self.kconv4_2(self.kconv4_1(f)))
#         #     DM = self.DME(f)
#         f_R_warped = DPD(Func.interpolate(input_img[:,:3,:,:], scale_factor=1 / 8, mode='area'), DM, padding_mode='zeros',
#                          device=self.device)
#         #     outs['f_R_w'] = f_R_warped
#         #     outs['f_L'] = Func.interpolate(L, scale_factor=1 / 8, mode='area')
#
#         return out, f_R_warped
...
class MYIR2(nn.Module):
    ...
    def forward(self, input, C):
        ...
        img_d8 = Func.interpolate(C, scale_factor=1 / 8, mode='area')
        ...
        est_img_d4_interpolate = Func.interpolate(est_img_d8, scale_factor=2, mode='area')
        ...
        est_img_d2_interpolate = Func.interpolate(est_img_d4, scale_factor=2, mode='area')
        ...
        est_img_d1_interploate = Func.interpolate(est_img_d2, scale_factor=2, mode='area')
...
if __name__ == '__main__':
    ...
    x = summary(model, [(6, 256, 256),(3, 256, 256)])
    a = 1
