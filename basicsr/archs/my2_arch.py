...
if __name__ == '__main__':
    model = MYIR2(num_heads=[ 3, 6, 12, 24 ],
                 embed_dim=96,
                 depths=[2, 2, 6, 2],
                 )
    model.cuda()
    ...
    resume_state = torch.load('/home/lab/code1/swinv2_tiny_patch4_window8_256.pth', map_location=lambda storage, loc: storage.cuda(device_id))

    model.load_state_dict(resume_state['model'], strict=False)
    for (name, param) in model.named_parameters():
        if name in resume_state['model']:
